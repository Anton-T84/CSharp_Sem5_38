...
2. Создать метод выявления минимального и максимального элемента массива.
3. Создать метод вывода массива.
4. Метод Ввода числового значения
    4.1 Универсальный метод для Размера массива и Диапазона минимального и максимального значения случайных чисел
*/
...
(int,int) minAndMaxOfArray(int[] array)
{ // метод поиска минимального и максимального значения
    int min = array[0];
    int max = array[0];
    for (int i = 0; i < array.Length; i++)
    {
...
int numberInput(string TextToWriteBeforeInput, string forArrayLengthOrForRandom)
{ //Вставил весь ввод чисел в один метод
...
// Так как метод ввода и проверки на число один, решил попробовать именные переменные для проверки читабельности кода.
